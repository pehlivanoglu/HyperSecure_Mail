...
def handle_sign_in(stdscr):
    stdscr.clear()
    email = prompt_user(stdscr, "Email: ")
    password = prompt_user(stdscr, "Password: ")
    token = signin(email, password)

    stdscr.clear()
    if isinstance(token, dict):
        stdscr.addstr("Successfully signed in.\nPress any key to continue...")
    else:
        stdscr.addstr(f"Sign in failed: {token}\nPress any key to retry...")
    stdscr.refresh()
    stdscr.getch()

def handle_sign_up(stdscr):
    stdscr.clear()
    email = prompt_user(stdscr, "New Email: ")
    password = prompt_user(stdscr, "Password: ")
    password_again = prompt_user(stdscr, "Enter Password Again: ")
    response = signup(email, password, password_again)

    stdscr.clear()
    if isinstance(response, dict):
        stdscr.addstr("Successfully signed up.\nPress any key to continue...")
    else:
        stdscr.addstr(f"Sign up failed: {response}\nPress any key to retry...")
    stdscr.refresh()
    stdscr.getch()
